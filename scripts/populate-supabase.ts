...
async function populateDatabase() {
  ...
  try {
    ...
    if (niguiriCat) {
      for (const product of niguiriProducts) {
        ...
        if (!existing) {
          ...
          if (error) console.error(`Erro ao adicionar ${product.name}:`, error);
          else console.log(`✅ Adicionado: ${product.name}`);
        }
      }
    }
    ...
    if (itemsForTradicional) {
      const settings = itemsForTradicional.map(item => ({
        group_id: 2,
        item_id: item.id,
        is_available: true,
        sort_order: 0
      ...
      if (error) console.error('Erro ao configurar Tradicional:', error);
      else console.log(`✅ Rodízio Tradicional configurado com ${settings.length} produtos`);
    }
    ...
    if (allItems) {
      ...
      for (let i = 0; i < alaCarteSettings.length; i += batchSize) {
        ...
        if (error) console.error(`Erro ao inserir lote ${i/batchSize + 1}:`, error);
        else console.log(`✅ Lote ${i/batchSize + 1} inserido (${batch.length} produtos)`);
      }
    ...
    const { data: traditionalCount } = await supabase
      .from('group_item_settings')
      .select('id', { count: 'exact' })
      .eq('group_id', 2);
    
    const { data: alaCarteCount } = await supabase
      .from('group_item_settings')
      .select('id', { count: 'exact' })
      .eq('group_id', 3);
    
    console.log('\n📊 Resultado Final:');
    console.log(`- Rodízio Premium: ${finalCount?.length || 0} produtos`);
    console.log(`- Rodízio Tradicional: ${traditionalCount?.length || 0} produtos`);
    console.log(`- À la Carte: ${alaCarteCount?.length || 0} produtos com preços`);
    
    console.log('\n✅ Banco de dados Supabase populado com sucesso!');
