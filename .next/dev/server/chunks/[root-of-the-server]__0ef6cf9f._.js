...
"[project]/app/api/mobile/service-types/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: serviceTypes, error } = await supabase.from('tablet_service_types').select(`
        *,
      ...
      `).eq('active', true).order('display_order');
        ...
        const formattedTypes = (serviceTypes || []).map((type)=>{
            ...
            const halfPriceFromGroup = linkedGroups.length > 0 && linkedGroups[0].half_price ? linkedGroups[0].half_price : null;
            return {
                id: type.id,
                name: type.name,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0ef6cf9f._.js.map