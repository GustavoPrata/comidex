...
"[project]/app/api/mobile/tablet-commands/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: commands, error } = await supabase.from('tablet_commands').select('*').or(`device_id.eq.${deviceId},target_all.eq.true`).eq('status', 'pending').order('created_at', {
            ascending: true
        }).limit(10);
...
async function POST(request) {
    try {
        ...
        if (!device_id && !target_all) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                success: false,
                error: 'device_id ou target_all é obrigatório'
            }, {
                status: 400
            });
        }
        const { data: newCommand, error } = await supabase.from('tablet_commands').insert({
            command,
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true,
            command: newCommand,
            message: target_all ? `Comando "${command}" enviado para todos os tablets` : `Comando "${command}" enviado para o tablet ${device_id}`
        });
    } catch (error) {
...
async function PATCH(request) {
    try {
        ...
        const { error } = await supabase.from('tablet_commands').update(updateData).eq('id', command_id);
        if (error) throw error;
        if (device_id) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e3887ff8._.js.map