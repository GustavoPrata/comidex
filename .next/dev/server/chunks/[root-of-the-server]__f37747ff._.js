...
"[project]/app/api/agent/jobs/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
;
const VALID_AGENT_TOKENS = process.env.AGENT_TOKENS?.split(',') || [
    'comidex-agent-2024'
];
    ...
    try {
      ...
      `).eq('status', 'pending').order('priority', {
        ...
        if (orderIds.length > 0) {
            const { data: orders } = await supabase.from('orders').select('id, table_session_id').in('id', orderIds);
            if (orders) {
        ...
        if (sessionIds.length > 0) {
            const { data: sessions } = await supabase.from('table_sessions').select(`id, table_id, tables (id, name)`).in('id', sessionIds);
            if (sessions) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f37747ff._.js.map