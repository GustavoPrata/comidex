...
function SettingsPage() {
    ...
    const saveSettings = async ()=>{
        ...
        try {
            if (settings.id) {
                // Update existing
                const { error } = await supabase.from('restaurant_settings').update(settings).eq('id', settings.id);
                if (error) throw error;
            } else {
                // Create new
            ...
            __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$react$2d$hot$2d$toast$2f$dist$2f$index$2e$mjs__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["toast"].error('Erro ao salvar configurações');
        } finally{
            setSaving(false);
        }
    };
