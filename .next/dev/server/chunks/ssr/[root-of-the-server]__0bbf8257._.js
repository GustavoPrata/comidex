...
function AdditionalsPage() {
    ...
    const duplicateAdditional = async (additional)=>{
        try {
            ...
            const { error } = await supabase.from('additionals').insert(newAdditional);
    ...
    const duplicateCategory = async (category)=>{
        try {
            ...
            const { error } = await supabase.from('additional_categories').insert(newCategory);
            if (error) throw error;
    ...
    const saveCategory = async ()=>{
        try {
            ...
            if (editingCategory) {
                const { error } = await supabase.from('additional_categories').update(categoryData).eq('id', editingCategory.id);
                if (error) throw error;
                ...
                const { error } = await supabase.from('additional_categories').insert(categoryData);
                if (error) throw error;
    ...
    const toggleActive = async (additional)=>{
        try {
            ...
            if (error) throw error;
            __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$sonner$2f$dist$2f$index$2e$mjs__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["toast"].success(`Adicional ${!additional.active ? 'ativado' : 'desativado'}`);
            loadData();
        } catch (error) {
