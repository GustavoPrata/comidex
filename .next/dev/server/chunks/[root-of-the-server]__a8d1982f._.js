...
"[project]/app/api/mobile/session/[id]/close/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function PUT(request, context) {
    try {
        ...
        // Buscar sessão ativa
        const { data: session, error: sessionError } = await supabase.from('tablet_sessoes').select(`
        *,
      ...
      `).eq('id', sessionId).eq('status', 'ativa').single();
        ...
        const { data: updatedSession, error: updateError } = await supabase.from('tablet_sessoes').update({
            status: 'finalizada',
        ...
        if (session.tablet_pedidos?.length > 0) {
            const orderIds = session.tablet_pedidos.map((p)=>p.id);
            await supabase.from('tablet_pedidos').update({
                status: 'finalizado'
            }).in('id', orderIds);
        }
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            session: {
                ...
                total: final_total,
                payment_method,
                items_count: session.tablet_pedidos?.reduce((acc, order)=>acc + (order.tablet_pedido_itens?.length || 0), 0) || 0
            },
...
function generateReceiptContent(session, totals) {
    const now = new Date();
    const items = [];
    // Coletar todos os itens de todos os pedidos
    session.tablet_pedidos?.forEach((order)=>{
        order.tablet_pedido_itens?.forEach((item)=>{
            items.push({
...
${items.map((item)=>`${item.quantity}x ${item.name}
   R$ ${item.price.toFixed(2)} = R$ ${item.total.toFixed(2)}`).join('\n')}
--------------------------------
...
async function DELETE(request, context) {
    try {
        ...
        // Cancelar sessão
        const { data: session, error } = await supabase.from('tablet_sessoes').update({
            status: 'cancelada',
            fim_atendimento: new Date().toISOString()
        }).eq('id', sessionId).select().single();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a8d1982f._.js.map