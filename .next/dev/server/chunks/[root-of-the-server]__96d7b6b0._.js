...
async function POST(req) {
    try {
        ...
        const { data, error } = await supabase.storage.from('restaurant').upload(fileName, buffer, {
        ...
        if (error) {
            ...
            if (error.message && (error.message.includes('bucket') || error.message.includes('not found'))) {
                ...
                const { data: retryData, error: retryError } = await supabase.storage.from('restaurant').upload(fileName, buffer, {
                    contentType: file.type || 'image/jpeg',
                    cacheControl: '3600',
                    upsert: false
                });
...
async function DELETE(req) {
    try {
        ...
        const { error } = await supabase.storage.from('restaurant').remove([
            filePath
        ]);
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__96d7b6b0._.js.map