...
async function POST(request) {
    try {
        ...
        const { data: existingSession } = await supabase.from('table_sessions').select().eq('table_id', body.table_id).eq('status', 'active').single();
        if (existingSession) {
...
async function PATCH(request) {
    try {
        ...
        const { data, error } = await supabase.from('table_sessions').update(body).eq('id', sessionId).select().single();
        if (error) throw error;
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(data);
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1b824402._.js.map