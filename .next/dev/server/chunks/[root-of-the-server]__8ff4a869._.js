...
"[project]/app/api/tablet/config/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        const { data: config, error } = await supabase.from('tablet_config').select('*').single();
...
async function POST(request) {
    try {
        ...
        const { data: existing } = await supabase.from('tablet_config').select('id').single();
        let result;
        if (existing) {
            // Atualizar configuração existente
            result = await supabase.from('tablet_config').update(data).eq('id', existing.id).select().single();
        } else {
            // Criar nova configuração
            result = await supabase.from('tablet_config').insert(data).select().single();
        }
        if (result.error) {
            throw result.error;
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8ff4a869._.js.map