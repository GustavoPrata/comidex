...
async function POST(request) {
    try {
        ...
        const { data: order, error: orderError } = await supabase.from('tablet_pedidos').insert({
            ...
            status: 'pendente',
            valor_total: total,
            observacoes: `Origem: mobile${device_id ? ` - Device: ${device_id}` : ''}`,
            created_at: new Date().toISOString()
        }).select().single();
        ...
        for (const item of items){
            ...
            const { data: product } = await supabase.from('items').select('*, categories(name)').eq('id', item.product_id).single();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__613fb962._.js.map