...
async function GET(request) {
    try {
        ...
        let query = supabase.from('items').select('*, categories(id, name, image, group_id)').eq('active', true).order('sort_order', {
            ascending: true
        });
        ...
        if (group_id) {
            products = products.filter((item)=>item.categories && item.categories.group_id === parseInt(group_id));
        }
