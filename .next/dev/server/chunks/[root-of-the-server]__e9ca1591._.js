...
"[project]/app/api/mobile/tablet-status/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const { error } = await supabase.from('registered_tablets').update(updateData).eq('device_id', device_id);
        if (error) throw error;
        const { data: commands } = await supabase.from('tablet_commands').select('*').or(`device_id.eq.${device_id},target_all.eq.true`).eq('status', 'pending').order('created_at', {
            ascending: true
        }).limit(5);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e9ca1591._.js.map